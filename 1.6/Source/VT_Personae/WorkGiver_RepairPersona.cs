...
namespace VT_Personae
{
	public class WorkGiver_RepairPersona : WorkGiver_Scanner
	{
		...
		{
			Pawn persona = (Pawn)t;
			if (persona == null || !persona.IsPersonaPawn())
			{
				return false;
			}
			...
			if (pawn != persona)
			{
				if (persona.CurJobDef == Extensions.RR_DefOf.RR_RepairPersonae)
				{
					return false;
				}
			...
			return JobDriver_RepairPersona.CanRepair(persona);
		}
		...
		{
			return JobMaker.MakeJob(Extensions.RR_DefOf.RR_RepairPersonae, t);
		}
	}
}