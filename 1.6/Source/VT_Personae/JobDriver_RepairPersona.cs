...
{
	public class JobDriver_RepairPersona : JobDriver
	{
		protected Pawn Persona => (Pawn)job.GetTarget(TargetIndex.A).Thing;

		protected int TicksPerHeal => Mathf.RoundToInt(1f / pawn.GetStatValue(Extensions.RR_DefOf.TendSpeed_Synth, true, -1) * 120f);
		protected int ticksToNextRepair;


		public override bool TryMakePreToilReservations(bool errorOnFailed)
		{
			return pawn.Reserve(Persona, job, 1, -1, null, errorOnFailed, false);
		}
		...
		protected override IEnumerable<Toil> MakeNewToils()
		{
			...
			int ticks = (int)(1f / pawn.GetStatValue(Extensions.RR_DefOf.TendSpeed_Synth) * 600f);
			...
			tendToil.WithEffect(Extensions.RR_DefOf.RepairingSynthetic, TargetIndex.A);
			...
			repairToil.WithEffect(Extensions.RR_DefOf.RepairingSynthetic, TargetIndex.A);
			...
			repairToil.tickAction = delegate
			{
				...
				{
					Persona.needs.food.CurLevel -= Persona.GetStatValue(Extensions.RR_DefOf.PersonaEnergyLossPerHP, true, -1);
					RepairTick(Persona);
					ticksToNextRepair = TicksPerHeal;
				}
				pawn.rotationTracker.FaceTarget(Persona);
			...
			{
				if (pawn == Persona && pawn.Faction != Faction.OfPlayer && pawn.IsHashIntervalTick(100, delta) && !pawn.Position.Fogged(pawn.Map))
				{
					FleckMaker.ThrowMetaIcon(pawn.Position, pawn.Map, Extensions.RR_DefOf.RR_PersonaFleckRepair, 0.42f);
				}
				if (pawn != Persona)
				{
					pawn.rotationTracker.FaceTarget(Persona);
